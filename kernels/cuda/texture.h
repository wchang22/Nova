...
__device__
inline float3 read_material(cudaTextureObject_t materials, const TriangleMetaData& meta,
...
__device__
inline float3 compute_normal(cudaTextureObject_t materials, const TriangleMetaData& meta,
...
__device__
inline float geometry_smith(float n_dot_v, float n_dot_l, float nvl, float roughness) {
...
__device__
inline float3 specularity(float3 view_dir, float3 half_dir, float3 diffuse, float metallic) {
...
__device__
inline float3 shade(float3 light_dir, float3 view_dir, float3 half_dir, float light_distance, 
