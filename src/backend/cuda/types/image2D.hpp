...
class Image2D {
public:
  Image2D() : buffer(nullptr), width(0), height(0) {}
  Image2D(size_t width, size_t height) : buffer(nullptr), width(width), height(height) {}
  ...
  virtual ~Image2D() { CUDA_CHECK(cudaFreeArray(buffer)) }
